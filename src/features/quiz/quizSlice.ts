...
export const updateQuizWithAnsweredQuestionThunk = createAsyncThunk(
  ...
  async (choiceSelectedPayLoad: any, thunkAPI: any) => {
    ...
    if (quizState.numberOfQuestionsAnswered === quizState.questions.length) {
      thunkAPI.dispatch(endQuiz());
    }
    return {
      quizState,
...
export const quizSlice = createSlice({
  ...
  reducers: {
    ...
    endQuiz: (state) => {
      ...
      state.dateString = currentDate.toDateString();
      state.totalTime = calculateGameTime(
        state.startDateInMilliseconds,
        currentDate.getTime()
      );
    },
...
export const { startQuiz, updateQuizWithAnsweredQuestion, resetQuiz, endQuiz } =
  quizSlice.actions;
